...
\begin{document}
	...
	\begin{tikzpicture}[declare function={%
		...
		\path (na.south) node[anchor=north,stylefont,yshift=-9pt] {Nguyên tử oxigen (O)};
		%%%%vẽ dấu cộng
		\path (4.5/8*d,0) node[stylefont] {+};
		%%%Vẽ nguyen tử thứ hai
		\path (6.5/8*d,0) node[stylefont] {2 e};
		%%%Vẽ mũi tên
		\path (9.5/8*d,0) node {\muiten};
		%%%Vẽ ion Olo
		\path (1.86*d,0) pic[local bounding box =nb,] {oxigenH};
		%%Vẽ khung 
		...
		\path(D) node[shift={(45:0.6cm)}]{\LARGE\textbf{2-}};
		\path (nb.south) node[anchor=north,stylefont,yshift=-4pt] {Ion oxigen (O$^\textbf{\LARGE 2-}$)};
	\end{tikzpicture}
\end{document}