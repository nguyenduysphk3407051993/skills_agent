...
\begin{document}
	...
	\begin{tikzpicture}[declare function={%
		...
		\tikzset{
			...
			FlourineH/.pic={
				...
				\foreach \i in {1,2,3,4,5,6}{
					\foreach \g/\r in{{\i*360/7}/r3h}{%
						\fill (\g:\r) [\maunhan!35!\mycolor] circle (4pt);
					}
				}
				%%%e ghép dôi
				\foreach \i in {7}{
					\foreach \g/\r in{{\i*360/7+5}/r3h-10.75pt}{%
						\fill (\g:\r) [\maunhan!35!\mycolor] circle (4pt);
					}
				}
			},
		...
		%%%Vẽ nguyen tu Fa
		\path (0,0) pic[local bounding box =Fa] {Flourine};
		%%%Vẽ dấu cộng
		\path (0.5*d,0) node[stylefont] {+};
		%%%Vẽ nguyen tu Fb
		\path (d,0) pic[local bounding box =Fb,rotate=180] {Flourine};
		%%%Vẽ nguyen tu Fc
		\path (2.5*d,0) pic[local bounding box =Fc] {FlourineH};
		%%%Vẽ nguyen tu Fd
		\path (3.15*d,0) pic[local bounding box =Fd,rotate=180] {FlourineH};
		%Vẽ mũi tên nhường nhận e 
		\path[draw=\mycolor, line width=6pt,-stealth,line cap=round,line join=round] ($(Fb.east)+(5pt,0)$)--($(Fc.west)+(-5pt,0)$) node [pos=0.5,above,font=\Large\bfseries\sffamily,text=\maunhan]{góp chung e};
	\end{tikzpicture}
\end{document}